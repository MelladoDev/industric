...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await useApi.post("/login", loginData);
      // Suponiendo que el token está en response.data.token
      const token = response.data.token;
      localStorage.setItem("token", token); // Almacena el token en localStorage
      navigate("/user-profile"); // Redirecciona al usuario a su perfil
    } catch (error) {
    ...
    <div className="flex flex-col items-center h-screen">
        <div className="w-full max-w-md px-4 py-8 mx-auto mt-16 bg-white rounded-lg shadow-md">
            <div className="text-center">
                <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Inicio de sesión</h2>
                <p className="text-sm text-gray-600 dark:text-gray-400">Ingresa con tu cuenta</p>
            </div>
